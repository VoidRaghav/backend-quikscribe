...
# Optional: Legacy bot deployment (skip if missing)
if [[ -f "$SCRIPTPATH/../google_bot/k8s/deployment.yaml" ]]; then
  kubectl apply -f "$SCRIPTPATH/../google_bot/k8s/deployment.yaml"
else
  echo "Skipping google_bot/k8s/deployment.yaml (not found)"
fi

# Worker (with KEDA ScaledObject in same file). Apply only if KEDA CRD exists.
if kubectl get crd scaledobjects.keda.sh >/dev/null 2>&1; then
  kubectl apply -f "$SCRIPTPATH/../google_bot/k8s/worker.yaml"
else
  echo "KEDA not detected (CRD scaledobjects.keda.sh missing). Skipping worker apply. Run k8s/install_addons.sh to install KEDA, then re-run this script."
fi
